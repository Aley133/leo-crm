...
def upgrade() -> None:
    op.add_column("kaspi_xml_feeds", sa.Column("workspace_id", sa.Integer(), nullable=True))
    ...
    op.create_foreign_key("fk_kaspi_xml_feeds_workspace_id", "kaspi_xml_feeds", "workspaces", ["workspace_id"], ["id"], ondelete="CASCADE")
    op.create_index("ix_kaspi_xml_feeds_workspace_id", "kaspi_xml_feeds", ["workspace_id"])


def downgrade() -> None:
    op.drop_index("ix_kaspi_xml_feeds_workspace_id", table_name="kaspi_xml_feeds")
    op.drop_constraint("fk_kaspi_xml_feeds_workspace_id", "kaspi_xml_feeds", type_="foreignkey")
