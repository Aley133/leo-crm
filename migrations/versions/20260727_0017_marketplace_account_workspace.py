...
def upgrade() -> None:
    with op.batch_alter_table("marketplace_accounts") as batch_op:
        ...
        batch_op.create_foreign_key(
            "fk_marketplace_accounts_workspace_id_workspaces",
            "workspaces",
            ["workspace_id"],
            ["id"],
            ondelete="RESTRICT",
        ...
        batch_op.drop_constraint(
            "uq_marketplace_account_provider_external",
            type_="unique",
        )
        batch_op.create_unique_constraint(
            "uq_marketplace_account_workspace_provider_external",
            ["workspace_id", "provider", "external_account_id"],
        )

    # Remove the database default after every existing row has been backfilled.
    with op.batch_alter_table("marketplace_accounts") as batch_op:
