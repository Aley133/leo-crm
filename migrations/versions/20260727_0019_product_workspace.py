...
def _single_column_unique_objects() -> tuple[list[str], list[str]]:
    ...
    for index in inspector.get_indexes("products"):
        ...
        name = index.get("name")
        if name and index.get("unique") is True and columns == ("kaspi_product_id",) and not index.get("duplicates_constraint"):
...
def upgrade() -> None:
    ...
    with op.batch_alter_table("products") as batch_op:
        ...
        batch_op.create_foreign_key("fk_products_workspace_id_workspaces", "workspaces", ["workspace_id"], ["id"], ondelete="RESTRICT")
        ...
        batch_op.create_unique_constraint("uq_products_workspace_kaspi_product_id", ["workspace_id", "kaspi_product_id"])
    with op.batch_alter_table("products") as batch_op:
...
def downgrade() -> None:
    inspector = sa.inspect(op.get_bind())
    unique_names = {constraint.get("name") for constraint in inspector.get_unique_constraints("products") if tuple(constraint.get("column_names") or ()) == ("workspace_id", "kaspi_product_id") and constraint.get("name")}
    with op.batch_alter_table("products") as batch_op:
