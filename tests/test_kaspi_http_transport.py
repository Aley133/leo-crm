...
def test_fetch_orders_builds_proven_bounded_request_and_parses_page() -> None:
    seen: dict = {}
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(
            200,
            json={
                "data": [{"type": "orders", "id": "order-1", "attributes": {"code": "996801988", "status": "NEW", "updatedAt": "2026-07-19T10:05:00Z"}}],
                "links": {"next": "https://kaspi.kz/shop/api/v2/orders?page%5Bnumber%5D=3&page%5Bsize%5D=1"},
            },
        )

    transport = KaspiHttpTransport(KaspiHttpSettings(api_token="secret"), client=_client(handler), clock=lambda: now)
    page = transport.fetch_orders(cursor="2", updated_after=datetime(2026, 7, 18, 10, 0, tzinfo=UTC), limit=1)

    request = seen["request"]
    assert request.headers["X-Auth-Token"] == "secret"
    assert request.headers["Accept"] == "application/vnd.api+json"
    assert request.headers["User-Agent"] == "leo-crm/0.8.1"
    ...
    assert request.url.params["filter[orders][creationDate][$le]"] == str(int(now.timestamp() * 1000))
    assert request.url.params["filter[orders][date][$ge]"] == "1784368800000"
    assert request.url.params["filter[orders][date][$le]"] == str(int(now.timestamp() * 1000))
...
def test_initial_request_uses_one_based_page_and_configured_lookback_window() -> None:
    ...
    transport = KaspiHttpTransport(KaspiHttpSettings(api_token="secret", initial_lookback_days=3), client=_client(handler), clock=lambda: now)
    transport.fetch_orders(cursor=None, updated_after=None, limit=10)

    params = seen["request"].url.params
    assert params["page[number]"] == "1"
    assert params["filter[orders][creationDate][$ge]"] == str(int((now - timedelta(days=3)).timestamp() * 1000))
    assert params["filter[orders][creationDate][$le]"] == str(int(now.timestamp() * 1000))


@pytest.mark.parametrize(
    ("status", "error_type"),
    [(401, KaspiAuthenticationError), (403, KaspiAuthenticationError), (429, KaspiRateLimitError), (503, KaspiTemporaryError), (400, KaspiTransportError)],
)
