...
class MarketplaceOrder(WorkspaceOwned, Base):
    ...
    manual_stage: Mapped[str | None] = mapped_column(String(32), index=True, nullable=True)
    manual_stage_reason: Mapped[str | None] = mapped_column(Text, nullable=True)
    manual_stage_updated_at: Mapped[datetime | None] = mapped_column(
        UTCDateTime(),
        nullable=True,
    )
