...
class KaspiHttpSettings:
    ...
    def from_environment(cls) -> "KaspiHttpSettings":
        ...
        try:
            timeout = float(os.getenv("KASPI_API_TIMEOUT_SECONDS", str(DEFAULT_KASPI_API_TIMEOUT_SECONDS)).strip())
        ...
        try:
            lookback_days = int(os.getenv("KASPI_INITIAL_LOOKBACK_DAYS", str(DEFAULT_KASPI_INITIAL_LOOKBACK_DAYS)).strip())
        except ValueError as exc:
...
class KaspiHttpTransport:
    def __init__(self, settings: KaspiHttpSettings, *, client: httpx.Client | None = None, clock: Callable[[], datetime] | None = None) -> None:
        if not settings.api_token.strip():
            raise KaspiConfigurationError("Kaspi API token is empty")
        self._settings = settings
        self._client = client or httpx.Client(timeout=httpx.Timeout(connect=10.0, read=settings.timeout_seconds, write=20.0, pool=60.0))
    ...
    def fetch_orders(self, *, cursor: str | None, updated_after: datetime | None, limit: int) -> MarketplaceOrderPage:
        ...
        window_start = self._as_utc(updated_after) if updated_after is not None else window_end - timedelta(days=self._settings.initial_lookback_days)
        ...
        try:
            response = self._client.get(f"{self._settings.base_url}/orders", params=params, headers={
                "Accept": "application/vnd.api+json",
                "Content-Type": "application/vnd.api+json",
                "X-Auth-Token": self._settings.api_token,
                "User-Agent": "leo-crm/0.8.1",
            })
        except (httpx.TimeoutException, httpx.NetworkError) as exc:
            raise KaspiTemporaryError(f"Kaspi request failed: {exc}") from exc
        if response.status_code in {401, 403}:
            raise KaspiAuthenticationError(f"Kaspi authentication failed with HTTP {response.status_code}")
        ...
        if response.status_code >= 400:
            raise KaspiTransportError(f"Kaspi request rejected with HTTP {response.status_code}")
        try:
        ...
        return MarketplaceOrderPage(items=items, next_cursor=self._next_cursor(document, page_number, len(items), limit), watermark_at=self._watermark(items))
    ...
    @staticmethod
    def _next_cursor(document: dict[str, Any], current_page: int, item_count: int, limit: int) -> str | None:
