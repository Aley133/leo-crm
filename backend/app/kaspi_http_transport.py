...
class KaspiHttpSettings:
    ...
    def from_environment(cls) -> "KaspiHttpSettings":
        ...
        return cls(api_token=token, base_url=base_url, timeout_seconds=timeout)


class KaspiHttpTransport:
    """Synchronous Kaspi order transport with no database responsibility.

    It performs one bounded JSON:API request. Retries are intentionally left to
    the outer scheduler so a database execution record can describe each try.
    """
    ...
    def fetch_orders(
        ...
        *,
        ...
        if updated_after is not None:
            aware = updated_after if updated_after.tzinfo is not None else updated_after.replace(tzinfo=UTC)
            params["filter[orders][creationDate][$ge]"] = int(aware.timestamp() * 1000)
        ...
        next_cursor = self._next_cursor(document, current_page=page_number, item_count=len(items), limit=limit)
        watermark = self._watermark(items)
        return MarketplaceOrderPage(items=items, next_cursor=next_cursor, watermark_at=watermark)
