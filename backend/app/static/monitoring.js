...
let activeRunsPollInFlight = false;
...
const labels = {
  ...
  waiting_for_agent:"Ожидает HTTP Agent", processing:"Выполняется", lease_expired:"Lease истёк",
...
const renderRuntimeHealth=()=>{
  ...
  const leased=cachedActiveRuns.filter((row)=>row.status==="processing").length;
  const stalled=cachedActiveRuns.filter((row)=>row.status==="lease_expired").length;
  ...
  if(stalled){state="bad";title=`${stalled} ${stalled===1?"проверка зависла":"проверки зависли"}`;detail="Lease истёк, но агент не подтвердил завершение задания.";action="Проверьте агент";}
  else if(unhealthy.length){state="bad";title=`Проблема с источником: ${unhealthy[0].supplier_name}`;detail=`Статус: ${labelFor(unhealthy[0].status)}. Ошибок подряд: ${unhealthy[0].consecutive_failures}.`;action="Проверьте источник";}
  ...
  else if(leased){state="active";title=`Выполняется ${leased} ${leased===1?"проверка":"проверки"}`;detail="Ozon отображается здесь в реальном времени.";action="HTTP Agent работает";}
  else if(queued){state="warn";title=`В очереди ${queued} ${queued===1?"задание":"задания"}`;detail="Задания ожидают свободный HTTP Agent и получение lease.";action="Ожидает агента";}
...
const sourceCell=(row)=>row.source_url?`<a href="${escapeHtml(row.source_url)}" target="_blank" rel="noreferrer">${escapeHtml(row.source_name||row.source_code)}</a>`:`<strong>${escapeHtml(row.source_name||row.source_code)}</strong>`;
const renderLeased=()=>{const rows=cachedActiveRuns;const body=document.querySelector("#leased-body");const table=document.querySelector("#leased-table");const empty=document.querySelector("#leased-empty");body.innerHTML=rows.map((row)=>`<tr class="${row.status==="lease_expired"?"active-run-stalled":""}"><td><strong>#${row.job_id}</strong><span class="muted">HTTP Agent</span></td><td>${row.product_id?`<a href="/crm/products/${row.product_id}">${escapeHtml(row.product_name||row.kaspi_product_id)}</a>`:escapeHtml(row.product_name||"—")}<span class="muted">${escapeHtml(row.merchant_sku?`SKU ${row.merchant_sku}`:row.kaspi_product_id||"")}</span></td><td>${sourceCell(row)}<span class="muted">${escapeHtml(row.source_code)}</span></td><td>${escapeHtml(row.agent_id||"—")}</td><td>${when(row.lease_until)}</td><td>${badge(row.status)}<strong class="run-elapsed" data-started-at="${escapeHtml(row.started_at)}">${elapsed(row.started_at)}</strong><span class="muted">${escapeHtml(row.detail||"")}</span></td></tr>`).join("");table.classList.toggle("hidden",rows.length===0);empty.classList.toggle("hidden",rows.length>0);const stalled=rows.filter((row)=>row.status==="lease_expired").length;setText("leased-label",stalled?`${rows.length} активных · ${stalled} требуют внимания`:`${rows.length} активных · автообновление`);setText("jobs-leased",rows.length);setText("agent-queue-count",cachedJobs.filter((row)=>row.status==="queued").length);setText("agent-last-work",cachedAttempts.length?when(cachedAttempts[0].started_at):"Нет данных");const durations=cachedAttempts.map((row)=>Number(row.duration_ms)).filter((value)=>Number.isFinite(value)&&value>=0);setText("agent-average-time",durations.length?formatDuration(durations.reduce((sum,value)=>sum+value,0)/durations.length):"Нет данных");setText("agent-idle-detail",cachedJobs.some((row)=>row.status==="queued")?"Свободный HTTP Agent пока не забрал задания из очереди.":"Активных проверок нет. HTTP Agent ожидает новые задания Ozon HTTP.");};
const tickActiveRunDurations=()=>document.querySelectorAll(".run-elapsed[data-started-at]").forEach((element)=>{element.textContent=elapsed(element.dataset.startedAt);});
...
const loadMonitoring=async()=>{const token=localStorage.getItem(storageKey);if(!token){authPanel.classList.remove("hidden");page.classList.add("hidden");return;}setLoading(true);message.textContent="";try{const [summary,jobs,attempts,sources,activeRuns]=await Promise.all([api("/api/monitoring-center/summary",token),api("/api/monitoring-center/jobs?limit=100",token),api("/api/monitoring-center/attempts?limit=100",token),api("/api/monitoring-center/sources",token),api("/api/monitoring-center/active-runs",token)]);cachedJobs=jobs;cachedAttempts=attempts;cachedSources=sources;cachedActiveRuns=activeRuns;setText("targets-total",summary.targets_total);setText("targets-active",summary.targets_active);setText("jobs-queued",summary.jobs_queued);rerender();renderSources(sources);document.querySelector("#jobs-updated").textContent=`Обновлено ${new Date().toLocaleTimeString("ru-RU",{hour:"2-digit",minute:"2-digit"})}`;authPanel.classList.add("hidden");page.classList.remove("hidden");}catch(error){message.textContent=error instanceof Error?error.message:"Не удалось загрузить мониторинг.";}finally{setLoading(false);}};
const pollActiveRuns=async()=>{const token=localStorage.getItem(storageKey);if(!token||document.hidden||activeRunsPollInFlight)return;activeRunsPollInFlight=true;try{cachedActiveRuns=await api("/api/monitoring-center/active-runs",token);renderRuntimeHealth();renderLeased();}catch{setText("leased-label","Нет свежих данных — повторяем запрос");}finally{activeRunsPollInFlight=false;}};
...
window.setInterval(tickActiveRunDurations,1000);
document.addEventListener("visibilitychange",()=>{if(!document.hidden)pollActiveRuns();});
