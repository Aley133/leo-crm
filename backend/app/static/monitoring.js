...
const renderRuntimeHealth=()=>{
  ...
  else if(leased){state="active";title=`Выполняется ${leased} ${leased===1?"проверка":"проверки"}`;detail="Ozon, Wildberries и Kaspi отображаются здесь в реальном времени.";action="Агенты работают";}
...
const renderLeased=()=>{const rows=cachedActiveRuns;const body=document.querySelector("#leased-body");const table=document.querySelector("#leased-table");const empty=document.querySelector("#leased-empty");body.innerHTML=rows.map((row)=>`<tr class="${row.status==="lease_expired"?"active-run-stalled":""}"><td><strong>#${row.job_id}</strong><span class="muted">${row.runtime==="kaspi_competitor"?"Kaspi Agent":"Browser Agent"}</span></td><td>${row.product_id?`<a href="/crm/products/${row.product_id}">${escapeHtml(row.product_name||row.kaspi_product_id)}</a>`:escapeHtml(row.product_name||"—")}<span class="muted">${escapeHtml(row.merchant_sku?`SKU ${row.merchant_sku}`:row.kaspi_product_id||"")}</span></td><td>${sourceCell(row)}<span class="muted">${escapeHtml(row.source_code)}</span></td><td>${escapeHtml(row.agent_id||"—")}</td><td>${when(row.lease_until)}</td><td>${badge(row.status)}<strong class="run-elapsed" data-started-at="${escapeHtml(row.started_at)}">${elapsed(row.started_at)}</strong><span class="muted">${escapeHtml(row.detail||"")}</span></td></tr>`).join("");table.classList.toggle("hidden",rows.length===0);empty.classList.toggle("hidden",rows.length>0);const stalled=rows.filter((row)=>row.status==="lease_expired").length;setText("leased-label",stalled?`${rows.length} активных · ${stalled} требуют внимания`:`${rows.length} активных · автообновление`);setText("jobs-leased",rows.length);setText("agent-queue-count",cachedJobs.filter((row)=>row.status==="queued").length);setText("agent-last-work",cachedAttempts.length?when(cachedAttempts[0].started_at):"Нет данных");const durations=cachedAttempts.map((row)=>Number(row.duration_ms)).filter((value)=>Number.isFinite(value)&&value>=0);setText("agent-average-time",durations.length?formatDuration(durations.reduce((sum,value)=>sum+value,0)/durations.length):"Нет данных");setText("agent-idle-detail",cachedJobs.some((row)=>row.status==="queued")?"Свободный агент пока не забрал задания из очереди.":"Активных проверок нет. Агенты ожидают новые задания.");};
