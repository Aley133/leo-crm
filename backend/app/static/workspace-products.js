...
const render = (rows) => {
  ...
  body.innerHTML = rows.map((row) => `
    <tr>
      <td><strong class="product-title">${escapeHtml(row.name)}</strong><span class="muted">Kaspi ${escapeHtml(row.kaspi_product_id)}${row.merchant_sku ? ` · SKU ${escapeHtml(row.merchant_sku)}` : ""}${row.brand ? ` · ${escapeHtml(row.brand)}` : ""}</span></td>
...
const xmlRequest = async (action, file) => {
  const response = await fetch(`/api/workspace/product-registry/imports/xml/${action}`, {
    method:"POST",
    headers:{...headers(),"Content-Type":"application/xml","X-Filename":encodeURIComponent(file.name)},
...
const renderXmlPreview = (payload) => {
  xmlPreview.innerHTML = `<div class="preview-grid"><article><span>Всего позиций</span><strong>${payload.total}</strong></article><article><span>Новых товаров</span><strong>${payload.new_count}</strong></article><article><span>Будут обновлены</span><strong>${payload.existing_count}</strong></article><article><span>Предупреждений</span><strong>${payload.warning_count}</strong></article></div><div class="preview-sample">${payload.sample.map((item) => `<div><strong>${escapeHtml(item.name)}</strong><span>Kaspi ${escapeHtml(item.kaspi_product_id)}${item.brand ? ` · ${escapeHtml(item.brand)}` : ""}</span></div>`).join("")}</div>`;
...
const previewXml = async (file) => {
  ...
  try {
    ...
    void xmlRequest("retain-source", file).catch(() => {});
  } catch (error) {
